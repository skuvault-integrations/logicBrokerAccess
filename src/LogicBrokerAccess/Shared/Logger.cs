...
namespace LogicBrokerAccess.Shared
{
	public class LogicBrokerAccessLogger
	{
		...
		{
			Log().Trace( exception, "{channel} An exception occured. [ver:{version}]", ChannelMark, _versionInfo );
		}
		...
		private static void TraceLog( string type, string info )
		{
			...
			{
				Log().Trace( "[{channel}] {type}:{info}, [ver:{version}]", ChannelMark, type, info, _versionInfo );
				return;
			}
			...
			{
				Log().Trace( "[{channel}] page:{page} pageId:{pageId} {type}:{info}, [ver:{version}]", ChannelMark, pageNumber++, pageId, type, page, _versionInfo );
			}
		}
